...
pub fn load(backing_allocator: Allocator, io: Io, environment: Environment) Allocator.Error!Loaded {
    ...
    const path = resolvePath(allocator, environment.home) catch |err| switch (err) {
        ...
        return .{
            .arena = arena,
            .snapshot = try diagnosticSnapshot(
                allocator,
                .file_read_failure,
    ...
    const snapshot = switch (parsed) {
        ...
        .malformed_json => try diagnosticSnapshot(
            allocator,
            .malformed_json,
            "user configuration contains malformed JSON",
            path,
        ),
        .invalid_schema => |schema_error| try diagnosticSnapshot(
            allocator,
            .invalid_schema,
...
fn diagnosticSnapshot(
    ...
    diagnostics[0] = .{ .code = code, .message = message, .path = path };
    return .{
        .configuration = .{ .object = .empty },
...
test "loader reads and serializes the user configuration file" {
    ...
    try std.testing.expectEqualStrings(
        "{\"configuration\":{\"keybindings\":{\"normal\":{\"focus.file_tree\":[[\"g\",\"t\"]]}}},\"diagnostics\":[]}",
        encoded,
    );
}
...
test "loader reports malformed JSON and invalid schema without failing" {
    ...
    try std.testing.expectEqual(@as(usize, 1), malformed.snapshot.diagnostics.len);
    try std.testing.expectEqual(
        DiagnosticCode.malformed_json,
        malformed.snapshot.diagnostics[0].code,
    );
    ...
    try std.testing.expectEqual(@as(usize, 1), invalid.snapshot.diagnostics.len);
    try std.testing.expectEqual(
        DiagnosticCode.invalid_schema,
        invalid.snapshot.diagnostics[0].code,
    );
...
test "missing configuration is empty while read failures are diagnosed" {
    ...
    try std.testing.expectEqual(@as(usize, 0), missing.snapshot.diagnostics.len);
    ...
    try std.testing.expectEqual(@as(usize, 1), unreadable.snapshot.diagnostics.len);
    try std.testing.expectEqual(
        DiagnosticCode.file_read_failure,
        unreadable.snapshot.diagnostics[0].code,
    );
}
