...
`comments.types` is an ordered list of comment type names. Without it, Rvw offers `ISSUE`, `QUESTION`, and `NITPICK`. A configured list replaces those built-ins; an empty list disables typed comments. `comments.defaultType` may be `null` or one of the configured names and defaults to `null`. Names must be unique, non-blank, single-line strings. In a comment text editor, `Tab` and `Shift+Tab` cycle through no type and the configured types. `Alt+Down` moves focus to the type dropdown, after which normal Tab navigation reaches the form buttons. The dropdown also supports mouse selection and clearing.
